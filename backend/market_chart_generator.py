...
def generate_market_chart(df, output_path):
    ...
    # --- Panel 1: 4h StochRSI (Replaces TSV) ---
    if '4h_Fast_K' in df.columns and '4h_Slow_D' in df.columns:
        # 80/20 lines for 4h
        ...
        apds.append(mpf.make_addplot(df['4h_Fast_K'], panel=1, color='cyan', width=1.5, ylabel='4h StochRSI'))
        apds.append(mpf.make_addplot(df['4h_Slow_D'], panel=1, color='orange', width=1.5))
    elif 'TSV' in df.columns:
        # Fallback to TSV if 4h StochRSI is missing
