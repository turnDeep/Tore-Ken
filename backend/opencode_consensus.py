...
def build_consensus_prompt(ranking_rows: list[dict[str, Any]], asof_date: str) -> str:
    rows_json = json.dumps(ranking_rows[:20], ensure_ascii=False, indent=2)
...
日本語で300字以内の要点に圧縮することです。

## 使うモデル

- Kimi: 事実関係、銘柄エンティティ、ニュース混入チェック
- MiniMax: 価格/出来高/需給の違和感チェック
- GLM: 業種、競合、構造テーマ、ファンダ文脈チェック
- DeepSeek: 上記3モデルと自身の監査を統合するまとめ役
...
- supply_risk は low / medium / high で弁別し、highだけを重大注意にする。
- thesis_mixed は mixed_strong / mixed_watch / entity_or_data_check / risk_dominant で補助分類する。
- daily_flags は日々の観測、thesis_state_change は重大イベントまたは3営業日継続でのみ昇格する。
