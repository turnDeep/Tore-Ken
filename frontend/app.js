...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Silver Futures Functions ---
    async function fetchSilverDataAndRender() {
        try {
            const response = await fetchWithAuth('/api/silver-analysis');
            if (!response.ok) throw new Error("Failed to load silver analysis");

            const data = await response.json();

            if (data.history && data.history.length > 0) {
                silverHistory = data.history;
                currentSilverDateIndex = silverHistory.length - 1;

                setupSilverControls();
                updateSilverDailyView(currentSilverDateIndex);
            } else {
                console.error("No silver history data found");
            }
        } catch (error) {
            console.error("Failed to fetch silver analysis:", error);
        }
    }

    function setupSilverControls() {
        const slider = document.getElementById('silver-date-slider');
        const prevBtn = document.getElementById('silver-prev-btn');
        const nextBtn = document.getElementById('silver-next-btn');

        if (!slider || !prevBtn || !nextBtn) return;

        slider.min = 0;
        slider.max = silverHistory.length - 1;
        slider.value = currentSilverDateIndex;

        slider.addEventListener('input', (e) => {
            const idx = parseInt(e.target.value);
            updateSilverDailyView(idx);
        });

        prevBtn.addEventListener('click', () => {
            if (currentSilverDateIndex > 0) {
                updateSilverDailyView(currentSilverDateIndex - 1);
                slider.value = currentSilverDateIndex;
            }
        });

        nextBtn.addEventListener('click', () => {
            if (currentSilverDateIndex < silverHistory.length - 1) {
                updateSilverDailyView(currentSilverDateIndex + 1);
                slider.value = currentSilverDateIndex;
            }
        });
    ...
    function updateSilverDailyView(index) {
        currentSilverDateIndex = index;
        const historyItem = silverHistory[index];

        // Update Date Display
        const dateEl = document.getElementById('silver-selected-date');
        if (dateEl) dateEl.textContent = historyItem.date;

        // Update Status Badge
        const badge = document.getElementById('silver-status-badge');
        if (badge) {
            badge.textContent = historyItem.status_text;
            badge.className = 'status-text'; // Reset
        ...
        // Move Vertical Line on Image
        const chartWrapper = document.getElementById('silver-chart-wrapper');
        const cursor = document.getElementById('silver-chart-cursor');

        if (chartWrapper && cursor && silverHistory.length > 0) {
            const marginLeft = 0.05;
            const marginRight = 0.12;
            const plotWidthPct = 1.0 - marginLeft - marginRight;

            const count = silverHistory.length;
            const pct = (index + 0.5) / count;
            const leftPos = (marginLeft + (pct * plotWidthPct)) * 100;
